...
const clients: Client[] = [
  ...
  { name: 'Shloimis', src: '/clients/shloimis.png' },
...
/** Uniform swipeable client logo row */
      ...
      >
        <div className="flex w-max items-center gap-0 px-6 md:px-10">
          {loop.map((client, i) => (
            <div
              key={`${client.name}-${i}`}
              className="flex h-[4.5rem] w-[12.5rem] shrink-0 items-center justify-center px-2 md:h-20 md:w-[12.5rem]"
              title={client.name}
            >
              ...
              <img
                ...
                width={200}
                height={72}
                className="h-[3.25rem] w-[12.5rem] object-contain object-center opacity-90 select-none transition duration-300 hover:opacity-100 md:h-14"
              />
            </div>
          ))}
        </div>
