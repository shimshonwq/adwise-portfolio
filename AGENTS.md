...
- **CMS**: Public pages load live content via `SiteContentProvider` → `GET /api/content/` (fallback `public/data/content.json` / `data/cms-default.json`). Logo bar uses visible logos in `order`. White strip, original colors, same slot (`15rem` × `5.25rem`), auto-slide + drag.
- **Private admin** at `/login` (not in nav, `noindex`) — multi-tab CMS: logos (reorder ↑↓, rename, hide, upload, remove), site/contact, hero, clients, services, spotlight, process, about, contact. `npm run dev` starts Next + `scripts/admin-api.mjs` (8787). Local password default `adwise-admin` (`ADMIN_PASSWORD`). Production: KV binding `LOGOS` + `wrangler secret put ADMIN_PASSWORD` (see README). Do not add `/login` to public nav.
