...
- **CMS**: Public pages load from `GET /api/content/` (GitHub when configured), fallback `public/data/content.json`. Admin at `/login` commits to **`public/data/content.json`** (+ logo files) via GitHub API — Cloudflare rebuilds on push. Local: `GITHUB_TOKEN` from `.env.local` or `gh auth token`. Production: Worker secrets `GITHUB_TOKEN`, `GITHUB_REPO`. Admin password hash in `data/admin-auth.json` (`npm run bootstrap:admin`). PBKDF2 uses 100k iterations (Cloudflare Worker Web Crypto limit). KV binding `LOGOS` is optional legacy fallback only. Deploy: `npm run deploy` then `npx wrangler versions deploy <version-id>`.
