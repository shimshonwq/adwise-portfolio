...
- **Contact emails**: Form posts to `POST /api/contact` (Worker). Default delivery is FormSubmit AJAX with `_captcha=false` (no FormSubmit redirect). Optional `RESEND_API_KEY` Worker secret is preferred when set. If FormSubmit asks for activation, open the Activate email sent to `site.email` once.
